...
import { challengesApi, statsApi, newsApi, eventsApi } from "@/lib/api";
...
export default function Home() {
  ...
  const {
    ...
    queryFn: () => newsApi.getAll(3),
  });
  const {
    data: events
  } = useQuery({
    queryKey: ["upcoming-events"],
    queryFn: () => eventsApi.getAll(3, true),
  });
  ...
  const {
    ...
    queryFn: () => challengesApi.getAll(), // You can add a limit if needed on backend
  });
